...
//     pub fn proxy_mint_pda_to(ctx: Context<ProxyMintPdaTo>, amount: u64) -> ProgramResult {
//         msg!("Minting");
...
//         };
//         let key = Pubkey::from_str("CLAKGbuuzgrQLizP2hyRVDEvKdJz2pr85X2HVyjjafYz").unwrap();
//         let signer_seeds = &[key.as_ref(), &[0]];
//         let signer = &[&signer_seeds[..]];

//         let cpi_program = ctx.accounts.token_program.clone();
//         let ctx = CpiContext::new(cpi_program, cpi_accounts).with_signer(signer);
...
//     pub fn pda(ctx: Context<Pda>, key: Pubkey) -> ProgramResult {
//         let signer_seeds = &[key.as_ref(), &[0]];
//         let authority_pubkey = Pubkey::create_program_address(signer_seeds, ctx.program_id)?;

//         msg!("Seeds {:?}", signer_seeds);
//         msg!("Authority {:?}", authority_pubkey);
//         Ok(())
//     }
...
//     pub fn proxy_pda_authority(ctx: Context<ProxySetAuthority>) -> ProgramResult {
//         let key = Pubkey::from_str("CLAKGbuuzgrQLizP2hyRVDEvKdJz2pr85X2HVyjjafYz").unwrap();
//         let signer_seeds = &[key.as_ref(), &[0]];
//         let authority_pubkey = Pubkey::create_program_address(signer_seeds, ctx.program_id)?;

//         let new_authority = Some(authority_pubkey);
...
// #[derive(Accounts)]
// pub struct ProxyMintPdaTo<'info> {
//     pub authority: AccountInfo<'info>,
//     #[account(mut)]
//     pub mint: AccountInfo<'info>,
//     #[account(mut)]
